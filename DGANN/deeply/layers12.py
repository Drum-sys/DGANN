...
class GraphAttentionLayer(Module):
    ...
    def forward(self, inp, adj):
        ...
        # # a_input = torch.cat([h.repeat(1, N).view(N*N, -1), h_2.repeat(N, 1)], dim=1).view(N, -1, 2*self.out_features)
        # # [N, N, 2*out_features]
        ...
        # # [N, N, 2*out_features]
        # e_2 = self.leakyrelu2(torch.matmul(a_input_2, self.a_2).squeeze(2))
        # # [N, N, 1] => [N, N] 图注意力的相关系数（未归一化）
        ...
        h_prime = torch.mm((torch.mm(attention, inp) + inp), self.W_3)
